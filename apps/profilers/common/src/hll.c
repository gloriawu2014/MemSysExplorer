...
void hll_reset(struct HLL *hll) {
    if (!hll) return;
    if (hll->registers) {
        memset(hll->registers, 0, hll->size);   /* size == #registers (m) */
    } else if (hll->bits) {
        /* In case someone calls reset before init: allocate lazily. */
        hll->size = (size_t)1 << hll->bits;
        hll->registers = calloc(hll->size, 1);
    }
}
